...
require 'json'
  ...
  def index
    if params[:keyword].present?
      ...
      @restaurants = Restaurant.where("LOWER(name) LIKE ?", "%#{keyword}%").paginate(:page => params[:page], :per_page => 100)
    else
      @restaurants = Restaurant.order("name asc").paginate(:page => params[:page], :per_page => 100)
    end
